...
class BasicTest extends TestCase
{
    ...
    public function setupQuery(Schema $schema, &$graph)
    {
        ...
        $query->field('pets')
            ->setFetcher(new CallbackFetcher(function (Node $node) use (&$graph) {
                $fetched = array_merge(
                    [],
                    array_values($this->dogs->toArray()),
                    array_values($this->cats->toArray())
                );
            ...
            ->setFetcher(new CallbackFetcher(function (Node $node) use (&$graph) {
                $fetched = array_merge(...array_map(function ($person) {
                    return array_values(array_merge(
                        array_filter($this->cats->toArray(), function ($cat) use ($person) {
                            return $cat->owner === $person->name;
                        }),
                        array_filter($this->dogs->toArray(), function ($dog) use ($person) {
    ...
    public function setupPet(Schema $schema, &$graph)
    {
        $animal = $schema->getType('Pet');
    ...
    public function setUp()
    {
        ...
        $this->dogs = Repositories::dogs();
        $this->cats = Repositories::cats();
        ...
        $schema->putType(new UnionType('Pet'));
